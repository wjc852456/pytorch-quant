...
def linear_quantize2(input, bits, delta):
    ...
    min_val = - bound
    max_val = bound - 1
    input = torch.floor(input / delta + 0.5)
    ...
    if isinstance(model,nn.Sequential):
        ...
        for i,(k,v) in ikv:
            if isinstance(v,nn.Conv2d):
                key,bn = next(ikv)[1]
                if isinstance(bn, nn.BatchNorm2d):
                    quant_layer = LinearQuant3(name='{}_quant'.format(k), 
                                                param_bits=param_bits, 
                                                fwd_bits=fwd_bits,
                                                conv=v, 
                                                bn=bn,
                                                counter=counter)
                    l['{}_linearquant3'.format(k)] = quant_layer
...
class LinearQuant2(nn.Module):
    ...
    def forward(self, input):
        if self._counter > 0:
            ...
            self.q_module.weight.data = linear_quantize2(self.module.weight, self.param_bits, self.delta_a)
            
            self.q_module.bias.data = torch.round( self.module.bias / (self.delta_a*self.delta_b) )
            ...
            #print(self.q_module.weight.view(-1))
            return q_output

class LinearQuant3(nn.Module):
    def __init__(self, name, param_bits, fwd_bits, conv, bn, delta_b=None, overflow_rate=0.0, counter=10):
        super(LinearQuant3, self).__init__()
        self.delta_b = delta_b
        self.delta_list = []
        self.name = name
        self._counter = counter
        ...
        self.q_module = copy.deepcopy(conv)
        self.q_module.bias = Parameter(self.bn.bias)
    ...
    def forward(self, input):
        if self._counter > 0:
            self._counter -= 1
            #print("self._counter:{}".format(self._counter))
            delta_b = compute_delta(input, self.fwd_bits, self.overflow_rate)
            self.delta_list.append(delta_b)
            ...
            if self.bn.affine:
                a = self.bn.weight / torch.sqrt(self.bn.running_var + self.bn.eps)
                b = - self.bn.weight * self.bn.running_mean / torch.sqrt(self.bn.running_var + self.bn.eps) + self.bn.bias
            else:
                a = 1.0 / torch.sqrt(self.bn.running_var + self.bn.eps)
                b = - self.bn.running_mean / torch.sqrt(self.bn.running_var + self.bn.eps)
            self.combine_weight = self.conv.weight * a.reshape(self.conv.out_channels,1,1,1)   #combine conv param with bn param to conv 
            self.delta_a      = compute_delta(self.combine_weight, self.param_bits, self.overflow_rate)
            self.quant_weight = linear_quantize2(self.combine_weight, self.param_bits, self.delta_a)

            self.q_module.weight.data =  self.quant_weight
            self.q_module.bias.data = torch.round(b / (self.delta_a*self.delta_b))
