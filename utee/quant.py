...
def compute_delta_perchannel(input, bits, overflow_rate=0):
    # compute delta
    in_size = len(input.size())
    if(in_size==4):
        N,C,W,H = input.size()
        C_in = input.permute(1,0,2,3).reshape([C,-1])
    elif(in_size==3):
        C,W,H = input.size()
        C_in = input.reshape([C,-1])
    else:
        N,_ = input.size()
        C = 1
        C_in = input.reshape([C,-1])
    S_in = C_in.abs().sort(dim=1,descending=True)[0]
    split_idx = int(overflow_rate * len(S_in[0]))
    v = S_in[:,split_idx] # shape C
    v = v.detach()
    si = torch.ceil(torch.log2(v+1e-12))
    sf = bits - 1 - si
    delta = torch.pow(2.0, -sf)
    return delta

def linear_quantize_perchannel(input, bits, delta):
    # quantize with delta
    in_size = len(input.size())
    if(in_size==4):
        N,C,W,H = input.size()
    ...
    min_val = - bound
    max_val = bound - 1
    Q_C_in = C_in / torch.unsqueeze(delta,1) + 0.5
    floored = QuantOp_floor.apply(Q_C_in) # size is C * -1
    quant_value = torch.unsqueeze(delta,1) * QuantOp_clamp.apply(floored, min_val, max_val)
    if(in_size==4):
        q_input = quant_value.reshape(C,N,W,H).permute(1,0,2,3)
    elif(in_size==3):
        q_input = quant_value.reshape(C,W,H)
    else:
...
class Linear_Quant(nn.Module):
    ...
    def forward(self, input):
        if self._counter > 0:
            self._counter -= 1
            delta_b = compute_delta(input, self.fwd_bits, self.overflow_rate)
            #delta_b = compute_delta_perchannel(input, self.fwd_bits, self.overflow_rate)
            self.delta_list.append(delta_b)
            if self._counter == 0:
                self.delta_list.sort()
                half = len(self.delta_list) // 2
            ...
            q_weight = linear_quantize2(self.weight, self.param_bits, self.delta_a)
            q_bias =  QuantOp_floor.apply( self.bias / (self.delta_a*self.delta_b) ) * self.delta_a * self.delta_b
            q_input = linear_quantize2(input, self.fwd_bits, self.delta_b)

            # per channel quantization
            #self.delta_a = compute_delta_perchannel(self.weight, self.param_bits, self.overflow_rate)
            #q_weight = linear_quantize_perchannel(self.weight, self.param_bits, self.delta_a)
            #q_bias = self.bias
            #q_input = linear_quantize_perchannel(input, self.fwd_bits, self.delta_b)
...
class Conv_BnQuant(nn.Module):
    ...
    def forward(self, input):
        if self._counter > 0:
            ...
            quant_weight = linear_quantize2(combine_weight, self.param_bits, delta_a)
            quant_bias = QuantOp_floor.apply(b / (delta_a*self.delta_b)) * delta_a * self.delta_b
            q_input = linear_quantize2(input, self.fwd_bits, self.delta_b)

            # per channel quantization
            #delta_a      = compute_delta_perchannel(combine_weight, self.param_bits, self.overflow_rate)
            #quant_weight = linear_quantize_perchannel(combine_weight, self.param_bits, delta_a)
            #quant_bias = b
            #q_input = linear_quantize_perchannel(input, self.fwd_bits, self.delta_b)
            ...
            q_output = F.conv2d(q_input, quant_weight, quant_bias, self.stride,
                        self.padding, self.dilation, self.groups)
            #print(quant_weight.contiguous().view(-1).topk(5))
